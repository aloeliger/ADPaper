...
def make_total_trigger_overlap_plot(
    ...
    fractions = np.array([
        pure_event_all_count.GetValue()/total_count.GetValue(),
        pure_event_count.GetValue()/total_count.GetValue(),
        impure_event_count.GetValue()/total_count.GetValue(),
        impure_event_all_count.GetValue()/total_count.GetValue(),
    ])
...
def sample_leading_trigger_overlap_plot(
    ...
    total_trigger_counts = sample.df.Count()
    trigger_counts = {}
    ...
    top_triggers = sortable_trigger_counts[-n_top_triggers:]

    top_trigger_names = [x[0] for x in top_triggers]
    top_trigger_counts = [x[1] for x in top_triggers]
    top_trigger_fractions = [x/total_trigger_counts.GetValue() for x in top_trigger_counts]
    # print()
    # print(sample_name)
    # print(top_trigger_names)
    # print(top_trigger_counts)
    # print(top_trigger_fractions)
    # print(total_trigger_counts.GetValue())
    # print()

    bins=np.arange(len(top_trigger_names))
    bin_edges = np.arange(len(top_trigger_names)+1)
    ...
    fig = hep.histplot(
        np.histogram(
            ...
            bins=bin_edges,
            #density=True,
            weights=top_trigger_fractions
        )
    )

    tick_positions = (bins+bins+1)/2
    plt.xticks(tick_positions, top_trigger_names, rotation='vertical')
    plt.ylabel('A.U.')
    plt.tight_layout()
