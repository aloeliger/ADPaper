...
def make_tower_variable(
        sample,
):
    sample.df = sample.df.Define(
        'emul_calo_tower_bx_0_mask',
        "L1EmulCaloTower_bx == 0"
    )
    sample.df = sample.df.Define(
        'nL1EmulCaloTower_BX0',
        "L1EmulCaloTower_bx[emul_calo_tower_bx_0_mask].size()"
    )

    return sample
    
def make_tower_comparison_plot(
        background_sample,
        background_sample_name,
        samples,
        score_name,
    ...
    #background_sample = make_tower_variable(background_sample)
    values = background_sample.df.AsNumpy([score_name, "nL1EmulCaloTower_BX0"])
    background_scores = values[score_name]
    background_ntowers = values["nL1EmulCaloTower_BX0"]

    for index, signal_sample_name in enumerate(samples):
        signal_sample = samples[signal_sample_name]
        #signal_sample = make_tower_variable(signal_sample)
        sample_df = signal_sample.df

        sample_values = sample_df.AsNumpy([score_name, "nL1EmulCaloTower_BX0"])
        sample_scores = sample_values[score_name]
        sample_ntowers = sample_values['nL1EmulCaloTower_BX0']
        ...
        fpr_towers, tpr_towers, _ = roc_curve(
            y_true,
            y_tower_scores,
        )
    ...
    hist_name = f'{score_display_name}_tower_comparison'
    plt.savefig(
        f'{output_path}/{hist_name}.png'
    )
    plt.savefig(
